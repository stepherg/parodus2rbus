    ...
    if(strcmp(op->valuestring, "GET")==0){
        ...
        cJSON_ArrayForEach(entry, params){
            if(cJSON_IsString(entry)){
                char* val = NULL; int rc = rbus_adapter_get(entry->valuestring, &val);
                if(rc==0 && val){
                    cJSON_AddStringToObject(results, entry->valuestring, val);
                    free(val);
                } else { failures++; cJSON_AddNullToObject(results, entry->valuestring); }
            } else {
